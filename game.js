...
const MUSIC={
  1:{step:.13,lead:[69,72,76,72,79,76,72,67,69,72,81,79,76,72,74,76],bossLead:[69,81,76,79,72,84,81,76,67,79,74,83,76,88,83,81],harmony:[57,null,60,null,64,null,60,null,57,null,64,null,62,null,64,null],bass:[45,null,45,null,48,null,43,null,45,null,41,null,43,null,40,null]},
  2:{step:.14,lead:[67,70,74,70,75,74,70,65,67,70,77,75,74,70,72,74],bossLead:[67,79,74,77,70,82,79,74,65,77,72,81,74,86,81,79],harmony:[55,null,58,null,62,null,58,null,55,null,62,null,60,null,62,null],bass:[43,null,43,null,46,null,41,null,43,null,39,null,41,null,38,null]},
  3:{step:.125,lead:[72,75,79,82,79,75,74,70,72,75,84,82,79,77,75,74],bossLead:[72,84,79,87,82,79,75,86,70,82,77,89,84,82,79,75],harmony:[60,null,63,null,67,null,62,null,60,null,67,null,65,null,63,null],bass:[48,null,46,null,43,null,46,null,48,null,41,null,43,null,46,null]},
  4:{step:.145,lead:[64,67,71,64,72,71,67,62,64,67,76,72,71,67,66,62],bossLead:[64,76,71,79,72,83,79,74,62,74,69,78,71,83,78,76],harmony:[52,null,55,null,59,null,55,null,52,null,59,null,57,null,55,null],bass:[40,null,40,null,43,null,38,null,40,null,36,null,38,null,35,null]}
};
const midiHz=note=>440*Math.pow(2,(note-69)/12);
function playMusicStep(){const track=MUSIC[state.level],bossEnemy=state.enemies.find(e=>TYPES[e.type].boss),boss=!!bossEnemy;if(state.musicBoss!==boss){state.musicBoss=boss;state.musicNote=0;tone(midiHz(boss?45:57),.28,boss?'sawtooth':'triangle',.016,'music')}const step=state.musicNote++,sequence=boss?track.bossLead:track.lead,index=step%sequence.length,phase=bossEnemy?.phase||1,lead=sequence[index],harmony=track.harmony[index],bass=track.bass[index],speed=boss?.78:1;if(lead!=null)tone(midiHz(lead+(boss&&phase===3&&index%4===3?12:0)),track.step*.82,'square',boss?.015:.012,'music');if(harmony!=null)tone(midiHz(harmony+(boss&&phase>1?5:0)),track.step*.72,index%4?'square':'sawtooth',boss?.008:.006,'music');if(bass!=null)tone(midiHz(bass-(boss&&phase===3&&index%8===4?12:0)),track.step*1.35,'triangle',boss?.022:.018,'music');if(boss&&index%2===1)tone(midiHz(track.bass[(index-1)%track.bass.length]||40)+.5,track.step*.34,'square',.004,'music');if(index%4===0||index%4===2||boss&&phase===3)chipNoise(index%4===0?.015:.009,index%4===0?.055:.025);state.musicNext=state.time+track.step*speed}
...
const CUTSCENES={
  ...
  outro1:[
    {image:'assets/calle-corrientes-4.jpg',overlay:'assets/peron.png',kicker:'BUENOS AIRES · PRINT ROOM',title:'THE MACHINE STOPS',speaker:'PERÓN',text:'You can defeat me. You cannot defeat the need to believe.',motion:'shake',accent:'red',cue:'threat'},
    {image:'assets/calle-corrientes-1.jpg',overlay:'assets/milei_heavy.png',kicker:'TEMPORAL ROUTE · OPEN',title:'BELIEF GETS A RECEIPT',speaker:'MILEI',text:'One machine down. Three historical mistakes to go.',motion:'snap',accent:'blue',cue:'victory'}
  ],
  outro2:[
    {image:'assets/ministry.jpg',overlay:'assets/mecha_fdr.png',kicker:'MINISTRY ROOF · DAWN',title:'THE EMERGENCY EXPIRES',speaker:'MECHA-FDR',text:'Without the crisis, what will they need us for?',motion:'drop',accent:'red',cue:'alarm'},
    {image:'assets/ministry.jpg',overlay:'assets/milei_heavy.png',kicker:'ROUTE 03 · PARIS 1968',title:'THAT WAS THE POINT',speaker:'MILEI',text:'The alphabet agencies scatter. The university signal is already waiting.',motion:'pan-left',accent:'blue',cue:'signal'}
  ],
  outro3:[
    {image:'assets/university.jpg',overlay:'assets/che_bike_idle.png',kicker:'CAMPUS EXIT · BLUE FIRE',title:'THE REVOLUTION MISSES ITS EXIT',speaker:"CHE'S GHOST",text:'An idea never dies.',motion:'shake',accent:'red',cue:'reveal'},
    {image:'assets/cs-milei-cockpit.jpg',kicker:'MOSCOW ROUTE · COMPROMISED',title:'NO MORE GUIDES',speaker:'MILEI',text:'Ideas have consequences. Betrayals have coordinates.',motion:'snap',accent:'blue',cue:'saw'}
  ],
  outro4:[
...
function cinematicBed(cue){if(!audio||state.muted||profile.music===false)return;const beds={calm:[110,165],alarm:[55,82],threat:[46,69],signal:[165,247],saw:[41,82],heartbeat:[48,72],reveal:[58,87],shop:[131,196],victory:[165,247]},[root,fifth]=beds[cue]||beds.signal;tone(root,1.35,'triangle',.009,'music');tone(fifth,1.05,'sine',.004,'music')}
...
const BOSS_INTROS={
  ...
  evita:{name:'EVITA',tag:'VOICE OF THE MACHINE',portrait:'evita.png',backdrop:'calle-corrientes-3.jpg',villain:'Listen to them. They know who loves them.',milei:'Love does not need loudspeakers.'},
  peron:{name:'JUAN PERÓN',tag:'THE STATE IN A SUIT',portrait:'peron.png',backdrop:'calle-corrientes-4.jpg',villain:'Argentina always returns to me.',milei:'I came for five dogs and one overdue correction.'},
  mecha_fdr:{name:'MECHA-FDR',tag:'THE PERMANENT EMERGENCY',portrait:'mecha_fdr.png',backdrop:'ministry.jpg',villain:'The crisis ends when I say it ends.',milei:'Then your term expires now.'},
...
function showTip(key,text){if(seenTips.has(key))return;seenTips.add(key);const match=text.match(/^(HOPPE|RAND|DOG SIGNAL):\s*(.*)$/);state.tipSource=match?.[1]||'';state.tip=match?.[2]||text;state.tipTime=match?5.8:5;if(match){tone(match[1]==='DOG SIGNAL'?72:match[1]==='RAND'?620:520,.07,'square',.026);setTimeout(()=>tone(match[1]==='DOG SIGNAL'?72:match[1]==='RAND'?780:660,.06,'square',.018),match[1]==='DOG SIGNAL'?150:80)}}
...
function spawnWave(){
  ...
  if(featured){state.bossQuote='';state.bossQuoteTime=0}
  ...
  const transmissions={'1-3':'HOPPE: THE RANSOM SIGNAL JUST MOVED EAST. KEEP PRESSING.','1-5':'HOPPE: THE PRINT ROOM IS CLOSE. THE SIGNAL IS GETTING LOUDER.','2-4':'RAND: EVERY AGENCY IS BUYING TIME FOR THE SAME MAN.','3-2':'HOPPE: MOSCOW IS LISTENING TO THIS FREQUENCY.','4-4':'DOG SIGNAL: FIVE HEARTBEATS · CORE LOCK FAILING'};if(transmissions[tipKey])showTip(`story-${tipKey}`,transmissions[tipKey]);
  state.bannerTime=2;
}
...
function update(dt){
  ...
  state.defeats.forEach(defeat=>defeat.life-=dt);state.defeats=state.defeats.filter(defeat=>defeat.life>0);
  state.enemies=state.enemies.filter(e=>{
    if(e.hp>0) return true;
    const bossDown=TYPES[e.type].boss;state.defeats.push({...e,life:bossDown?.8:.48,maxLife:bossDown?.8:.48});state.stats.enemiesDefeated=(state.stats.enemiesDefeated||0)+1;dropPeso(e);burst(e.x,e.y-45,'#ffd15a',bossDown?34:e.elite?24:16);popup(e.x,e.y-105,bossDown?'BOSS DOWN':e.elite?'ELITE AFUERA':'AFUERA','#ffd15a');state.shake=bossDown?14:e.elite?9:6;if(bossDown){state.hitStop=.12;rumble(1,180);tone(48,.55,'sawtooth',.065)}else tone(e.elite?72:96,.13,'square',.025);state.score+=Math.round((bossDown?5000:e.elite?1250:500)*state.multiplier); return false;
  });
...
function updateEnemies(dt){
  ...
  for(const e of state.enemies){
    ...
    if(t.boss){const next=e.hp/e.maxHp<=.33?3:e.hp/e.maxHp<=.66?2:1;if(next>e.phase){e.phase=next;e.attackCd=.75;state.hitStop=.09;state.phaseFlash=.8;state.shake=16;rumble(.9,150);state.banner=`${e.type==='gremialista'?'GREMIALISTA':e.type.toUpperCase()} · PHASE ${next}`;state.bannerTime=1.4;state.bossQuote=BOSS_PHASE_LINES[e.type]?.[next-2]||'';state.bossQuoteTime=2.5;burst(e.x,e.y-55,next===3?'#c62f37':'#e0ad3b',34);tone(82,.35,'sawtooth',.06);if(e.type==='super_stalin'&&next===3)playCutscene('marx')}}
    if(t.boss&&state.phaseFlash>.32){e.state='telegraph';e.vx=approach(e.vx,0,1100*dt);e.vy=approach(e.vy,0,1100*dt);continue}
    if(e.hit>0) continue;
...
function enemyStrike(e,t){e.state='attack';e.vx=0;e.vy=0;e.timer=.35;e.attackKind='strike';e.attackCd=(t.boss?Math.max(.42,.9-(e.phase-1)*.17):1.05+Math.random()*.55)/(selectedDifficulty==='motosierra'?1.12:1);let landed=false;if(e.type==='peron'&&e.phase>=2)spawnProjectile(e,'decree',t.damage*.7);const inRange=dist(e,hero)<t.range+28;if(inRange&&hero.invuln>0)perfectDodge(e.x,e.y-40);else if(inRange){landed=true;const damage=t.damage*(t.boss?1+(e.phase-1)*.16:1)*(e.elite?1.2:1)*difficultyDamage();hero.hp-=damage;hero.hit=.22;breakMultiplier();rumble(t.boss ? .7 : .42,t.boss ? 110 : 65);popup(hero.x,hero.y-85,`-${Math.round(damage)}`,'#ff6b68');hero.invuln=.5;hero.x+=e.face*30;state.shake=6;burst(hero.x,hero.y-35,'#c62f37',8);tone(70,.13,'square',.05)}if(t.thief&&landed&&state.pesos>0){const stolen=Math.min(20,state.pesos);state.pesos-=stolen;e.stolen=(e.stolen||0)+stolen;e.flee=2.7;e.x+=e.face*-55;popup(e.x,e.y-105,`STOLE $${Math.floor(stolen)}`,'#ffd15a');state.banner='TAX COLLECTOR ESCAPING';state.bannerTime=.8}if(hero.hp<=0)lose()}

function spawnProjectile(e,kind,damage){const dx=hero.x-e.x,dy=(hero.y-35)-(e.y-45),d=Math.hypot(dx,dy)||1,speed=kind==='decree'?235:kind==='missile'?270:kind==='lightning'?310:190,arm=kind==='missile'?.24:kind==='lightning'?.16:.08;state.projectiles.push({x:e.x,y:e.y-45,vx:dx/d*speed,vy:dy/d*speed,damage,kind,life:3,arm,maxArm:arm,targetX:hero.x,targetY:hero.y-8})}
function updateProjectiles(dt){state.projectiles.forEach(p=>{p.life-=dt;if(p.arm>0){p.arm=Math.max(0,p.arm-dt);return}p.x+=p.vx*dt;p.y+=p.vy*dt;const collision=Math.hypot(p.x-hero.x,p.y-(hero.y-35))<30;if(collision&&hero.invuln>0){perfectDodge(p.x,p.y);p.life=0}else if(collision){const damage=p.damage*difficultyDamage();hero.hp-=damage;hero.hit=.22;breakMultiplier();hero.invuln=.55;p.life=0;state.shake=5;rumble(.55,75);popup(hero.x,hero.y-85,`-${Math.round(damage)}`,'#ff6b68');burst(hero.x,hero.y-35,p.kind==='decree'?'#e0ad3b':'#73c8e8',9);tone(72,.12);if(hero.hp<=0)lose()}});state.projectiles=state.projectiles.filter(p=>p.life>0&&p.x>-40&&p.x<W+40&&p.y>240&&p.y<H+40)}
...
function updatePickups(dt){
  state.pickups.forEach(p=>{
    p.age+=dt;const pickupDistance=dist(p,hero),magnetRange=90+profile.magnet*38;if(pickupDistance<magnetRange&&pickupDistance>1){const pull=clamp(1-pickupDistance/magnetRange,.12,1),speed=(p.power?290:230)+profile.magnet*55;p.x+=(hero.x-p.x)/pickupDistance*speed*pull*dt;p.y+=(hero.y-p.y)/pickupDistance*speed*pull*dt;p.magnet=Math.min(1,(p.magnet||0)+dt*5)}else p.magnet=Math.max(0,(p.magnet||0)-dt*3);
    if(p.power){if(hit(p,hero,38+profile.magnet*20)){const color=p.power==='gold'?'#ffd15a':p.power==='hand'?'#73c8e8':'#78bd70';if(p.power==='gold'){hero.goldTimer=10;popup(hero.x,hero.y-100,'DOUBLE DAMAGE','#ffd15a')}else if(p.power==='hand'){hero.handTimer=6;popup(hero.x,hero.y-100,'INVISIBLE HAND','#73c8e8')}else{hero.hp=Math.min(hero.maxHp,hero.hp+35);popup(hero.x,hero.y-100,'MATE +35','#78bd70')}p.dead=true;state.banner=p.power==='gold'?'GOLDEN CHAINSAW':p.power==='hand'?'INVISIBLE HAND':'MATE BREAK';state.bannerTime=1;state.shake=Math.max(state.shake,3);burst(hero.x,hero.y-35,color,16);tone(760,.2,'square',.045);setTimeout(()=>tone(1040,.09,'square',.025),70)}return}
...
function updateHazards(dt){for(const h of state.hazards){if(h.dead)continue;h.damageCd=Math.max(0,(h.damageCd||0)-dt);const {active}=hazardState(h);if(active&&h.damageCd<=0&&hero.invuln<=0&&dist(h,hero)<h.r){const damage=h.type==='vent'?12:7;hero.hp-=damage;hero.hit=.22;breakMultiplier();hero.invuln=.65;h.damageCd=.8;state.shake=5;popup(hero.x,hero.y-85,`-${damage}`,h.type==='vent'?'#73c8e8':'#c62f37');burst(hero.x,hero.y-20,h.type==='vent'?'#73c8e8':'#c62f37',10);tone(h.type==='vent'?150:90,.16,'sawtooth',.05);if(hero.hp<=0)lose()}}state.hazards=state.hazards.filter(h=>!h.dead)}
...
const mechanicalLose=lose;lose=function(){mechanicalLose();const el=document.querySelector('#message'),canContinue=runContinues>0,stakes={1:'The ransom trail keeps moving. Every second makes the money—and the scent—colder.',2:'The Ministry seals another stairwell. The route to the dogs remains above you.',3:'The kennel signal loses another minute. Five heartbeats are still answering.',4:'Five heartbeats remain above the core. Stalin only needs you to stay down.'};if(bossPractice)return;const copy=el.querySelector('h1')?.nextElementSibling;if(copy)copy.textContent=canContinue?stakes[state.level]:'The rescue ends here only if you leave it here.';const button=el.querySelector('[data-continue]');if(button)button.textContent=`GET BACK UP · ${runContinues} LEFT`};
...
function showOutro(){if(state.level===3&&!seenScenes.has('betrayal')){playCutscene('betrayal',showOutro);return}const outroKey=`outro${state.level}`;if(!seenScenes.has(outroKey)){playCutscene(outroKey,showOutro);return}const el=document.querySelector('#message'),final=state.level===4;el.innerHTML=`<p class="eyebrow">${final?'HISTORY RESTORED':`LEVEL ${state.level} SECURED`}</p><h1>${final?'THE ROAD HOME IS OPEN.':'ROUTE ACQUIRED.'}</h1><p>${final?'The dogs are free. The timeline is snapping back into place.':'The next coordinates are locked. Bank the surviving value before the temporal door closes.'}</p><button>${final?'VIVA LA LIBERTAD':'ENTER THE FREE PORT'}</button>`;el.classList.remove('hidden');el.querySelector('button').onclick=final?showEnding:showShop}
...
const renderFreePort=showShop;showShop=function(){renderFreePort();if(document.querySelector('.free-port'))playShopMotif()};document.querySelector('#message').addEventListener('click',event=>{if(event.target.closest('.free-port')&&(event.target.closest('[data-next]')||event.target.closest('[data-replay]')))shopMusicActive=false},{capture:true});
const showMissionCard=showIntro;showIntro=function(level=1){if(level===1&&!seenScenes.has('theft')){showMissionCard(level);return}const key=`mission${level}`;if(CUTSCENES[key]&&!seenScenes.has(key)){playCutscene(key,()=>showMissionCard(level));return}showMissionCard(level)};
...
function drawForegroundMotion(){if(profile.reducedMotion)return;ctx.save();if(state.level===1){const sweep=(state.time*95)%430;ctx.globalAlpha=.16;ctx.fillStyle='#e0ad3b';for(let i=0;i<3;i++){ctx.save();ctx.translate((sweep+i*360)%(W+180)-90,H-32);ctx.transform(1,0,-.65,1,0,0);ctx.fillRect(-70,0,140,7);ctx.restore()}}else if(state.level===2){ctx.globalAlpha=.24;for(let i=0;i<5;i++){const x=(i*223+state.time*34)%(W+80)-40,y=H-24-Math.sin(state.time*2+i)*13;ctx.save();ctx.translate(x,y);ctx.rotate(state.time*.35+i);ctx.fillStyle='#f1e2bd';ctx.fillRect(-11,-6,22,12);ctx.fillStyle='#c62f37';ctx.fillRect(-7,-2,14,2);ctx.restore()}}else if(state.level===3){ctx.globalAlpha=.18;ctx.fillStyle='#c62f37';for(let i=0;i<4;i++){const x=i*290-(state.time*18)%290;ctx.beginPath();ctx.moveTo(x,H);ctx.lineTo(x+85,H);ctx.lineTo(x+65,H-48-Math.sin(state.time*2+i)*8);ctx.lineTo(x+10,H-42);ctx.fill()}}else{ctx.globalAlpha=.2;ctx.fillStyle='#73c8e8';for(let i=0;i<4;i++){const x=120+i*270,y=H-10-((state.time*70+i*43)%150);ctx.beginPath();ctx.arc(x+Math.sin(state.time+i)*18,y,16+i*2,0,Math.PI*2);ctx.fill()}}ctx.restore()}
...
function drawDefeats(){for(const defeat of state.defeats){const progress=1-defeat.life/defeat.maxLife,name=defeat.type==='super_stalin'?(defeat.phase===3?'stalin_mecha':defeat.phase===2?'stalin_exo':'stalin_form1'):defeat.type==='che_bike'?'che_bike_idle':defeat.type,im=images[name],scale=TYPES[defeat.type].scale*(.9+defeat.y/floorBottom()*.1);ctx.save();ctx.globalAlpha=clamp(defeat.life/defeat.maxLife,0,1);ctx.translate(defeat.x+(defeat.face||1)*progress*42,defeat.y-progress*34);ctx.rotate((defeat.face||1)*progress*.72);ctx.scale((defeat.face||1)*scale*(1+progress*.12),scale*(1-progress*.18));if(im?.complete)ctx.drawImage(im,-im.width/2,-im.height,im.width,im.height);ctx.restore()}}
...
function drawPeso(p){const decay=clamp(1-p.age/18,.2,1),urgent=p.age>11,pull=p.magnet||0;ctx.save();ctx.globalAlpha=decay;ctx.translate(p.x,p.y-Math.sin(state.time*7+p.x)*2);ctx.rotate(state.time*(2+pull*5)+p.x);if(pull){ctx.shadowColor=urgent?'#c62f37':'#78bd70';ctx.shadowBlur=10+pull*12}ctx.fillStyle=urgent?'#c62f37':p.age>6?'#e0ad3b':'#78bd70';ctx.fillRect(-12-pull*2,-7-pull,24+pull*4,14+pull*2);ctx.shadowBlur=0;ctx.fillStyle='#07172d';ctx.font='bold 7px Arial';ctx.textAlign='center';ctx.fillText(`$${Math.max(1,Math.floor(p.value))}`,0,3);ctx.restore();}
function drawPowerup(p){ctx.save();ctx.translate(p.x,p.y-20-Math.sin(state.time*4)*5);const color=p.power==='gold'?'#ffd15a':p.power==='hand'?'#73c8e8':'#78bd70',pull=p.magnet||0,pulse=1+Math.sin(state.time*7)*.05+pull*.12;ctx.scale(pulse,pulse);ctx.rotate(pull*Math.sin(state.time*12)*.08);ctx.shadowColor=color;ctx.shadowBlur=18+pull*18;ctx.fillStyle='#07172d';ctx.strokeStyle=color;ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,24,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle=color;ctx.textAlign='center';ctx.font='bold 13px Arial';ctx.fillText(p.power==='gold'?'2X':p.power==='hand'?'HAND':'MATE',0,5);ctx.restore()}
function drawFx(){const boss=state.enemies.find(e=>TYPES[e.type].boss);if(boss&&state.phaseFlash>0){const progress=1-state.phaseFlash/.8,radius=35+progress*235;ctx.save();ctx.globalAlpha=clamp(state.phaseFlash,0,.8);ctx.strokeStyle=boss.phase===3?'#c62f37':'#ffd15a';ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=18;ctx.lineWidth=12*(1-progress)+2;ctx.beginPath();ctx.ellipse(boss.x,boss.y-35,radius,radius*.38,0,0,Math.PI*2);ctx.stroke();ctx.restore()}for(const p of state.projectiles){ctx.save();if(p.arm>0){const readiness=1-p.arm/p.maxArm,color=p.kind==='missile'?'#ffd15a':'#73c8e8';ctx.globalAlpha=.38+readiness*.5;ctx.strokeStyle=color;ctx.lineWidth=2+readiness*4;ctx.setLineDash([8,6]);ctx.beginPath();ctx.ellipse(p.targetX,p.targetY,34-readiness*11,12-readiness*3,0,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=.45+readiness*.5;ctx.fillStyle=color;ctx.beginPath();ctx.arc(p.x,p.y,8+readiness*7,0,Math.PI*2);ctx.fill();ctx.restore();continue}const speed=Math.hypot(p.vx,p.vy)||1,tx=-p.vx/speed,ty=-p.vy/speed;ctx.strokeStyle=p.kind==='missile'?'#ffd15a88':p.kind==='decree'?'#f1e2bd66':'#73c8e888';ctx.lineWidth=p.kind==='lightning'?7:4;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(p.x+tx*(p.kind==='missile'?42:26),p.y+ty*(p.kind==='missile'?42:26));ctx.stroke();ctx.translate(p.x,p.y);ctx.rotate(Math.atan2(p.vy,p.vx));ctx.fillStyle=p.kind==='decree'?'#f1e2bd':p.kind==='missile'?'#c62f37':'#73c8e8';if(p.kind==='decree'){ctx.fillRect(-17,-12,34,24);ctx.strokeStyle='#c62f37';ctx.lineWidth=3;ctx.strokeRect(-17,-12,34,24);ctx.fillStyle='#07172d';ctx.font='bold 8px Arial';ctx.textAlign='center';ctx.fillText('DECREE',0,3)}else if(p.kind==='missile'){ctx.fillRect(-18,-6,28,12);ctx.fillStyle='#ffd15a';ctx.beginPath();ctx.moveTo(14,-8);ctx.lineTo(28,0);ctx.lineTo(14,8);ctx.fill()}else if(p.kind==='lightning'){ctx.strokeStyle='#73c8e8';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(-18,-10);ctx.lineTo(-5,7);ctx.lineTo(5,-8);ctx.lineTo(18,10);ctx.stroke()}else{ctx.beginPath();ctx.arc(0,0,12,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#f1e2bd';ctx.lineWidth=3;ctx.stroke()}ctx.restore()}for(const p of state.particles){ctx.globalAlpha=clamp(p.life*3,0,1);ctx.fillStyle=p.color;ctx.fillRect(p.x,p.y,5,5)}ctx.globalAlpha=1;for(const p of state.popups){ctx.globalAlpha=clamp(p.life*2,0,1);ctx.fillStyle=p.color;ctx.strokeStyle='#07172d';ctx.lineWidth=4;ctx.font='bold 22px Arial';ctx.textAlign='center';ctx.strokeText(p.text,p.x,p.y);ctx.fillText(p.text,p.x,p.y)}ctx.textAlign='left';ctx.globalAlpha=1;}
...
function drawHud(){
  ...
  if(state.tipTime>0){const tipY=matchMedia('(pointer:coarse), (max-width:800px)').matches?130:H-101,transmission=!!state.tipSource,panelX=transmission?120:160,panelW=transmission?W-240:W-320;ctx.globalAlpha=clamp(state.tipTime,0,1);ctx.fillStyle='#07172df2';ctx.fillRect(panelX,tipY,panelW,59);ctx.strokeStyle=transmission?'#e0ad3b':'#73c8e8';ctx.lineWidth=2;ctx.strokeRect(panelX,tipY,panelW,59);let textX=panelX+15;if(transmission){const portrait=state.tipSource==='RAND'?transmissionImages.RAND:state.tipSource==='DOG SIGNAL'?transmissionImages.DOG:null;ctx.fillStyle='#020712';ctx.fillRect(panelX+5,tipY+5,67,49);if(portrait?.complete)ctx.drawImage(portrait,0,0,portrait.width,portrait.height,panelX+5,tipY+5,67,49);else{ctx.strokeStyle='#73c8e8';ctx.lineWidth=2;ctx.beginPath();for(let i=0;i<11;i++){const x=panelX+9+i*5.5,y=tipY+29+Math.sin(state.time*9+i)*9;ctx.lineTo(x,y)}ctx.stroke()}textX=panelX+82}ctx.fillStyle=transmission?'#e0ad3b':'#73c8e8';ctx.font='10px Bungee';ctx.fillText(transmission?`${state.tipSource} · LIVE SIGNAL`:'LIBERTY INTEL',textX,tipY+21);ctx.fillStyle='#f1e2bd';ctx.font='bold 13px Chakra Petch';ctx.fillText(state.tip,textX,tipY+43);ctx.globalAlpha=1}
  ...
  if(state.sceneFade>0){const sceneNames={1:['AVENIDA 9 DE JULIO','CALLE CORRIENTES','PRINT DISTRICT','CASA ROSADA'],2:['RECEPTION','PERMIT FLOOR','ALPHABET WING','ROOFTOP'],3:['MAIN QUAD','LECTURE HALL','OCCUPIED LIBRARY','MOTOR POOL'],4:['OUTER RING','REACTOR FLOOR','KENNEL BLOCK','CORE CHAMBER']},label=sceneNames[state.level]?.[state.scene]||`SECTOR ${state.scene+1}`;ctx.globalAlpha=state.sceneFade;ctx.fillStyle='#050b14';ctx.fillRect(0,0,W,H);if(state.sceneFade>.3){ctx.textAlign='center';ctx.fillStyle='#73c8e8';ctx.font='11px Bungee';ctx.fillText(`ROUTE ${state.level}.${state.scene+1}`,W/2,H/2-14);ctx.fillStyle='#e0ad3b';ctx.font='27px Bungee';ctx.fillText(label,W/2,H/2+18);ctx.textAlign='left'}ctx.globalAlpha=1;}
