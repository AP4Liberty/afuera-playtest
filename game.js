...
let audio;
function tone(freq,duration=.08,type='square',volume=.035,channel='sfx'){if(!audio||state.muted||profile[channel]===false)return;const o=audio.createOscillator(),g=audio.createGain();o.type=type;o.frequency.setValueAtTime(freq,audio.currentTime);o.frequency.exponentialRampToValueAtTime(Math.max(40,freq*.65),audio.currentTime+duration);g.gain.setValueAtTime(volume,audio.currentTime);g.gain.exponentialRampToValueAtTime(.001,audio.currentTime+duration);o.connect(g).connect(audio.destination);o.start();o.stop(audio.currentTime+duration)}
...
const MUSIC={1:[110,147,165,147,123,165,196,147],2:[98,131,147,131,110,147,165,131],3:[123,155,185,207,185,155,138,155],4:[82,110,123,92,82,131,110,92]};
...
const CUTSCENES={
  theft:[
    {image:'assets/cs-calm-night-v2.jpg',kicker:'CASA ROSADA · 11:47 P.M.',title:'ONE QUIET HOUR',text:'The last meeting is over. Buenos Aires is asleep. Milei steps onto the balcony with the five creatures who never ask for a cabinet position.'},
    {image:'assets/cs-dog-theft-v2.jpg',kicker:'CASA ROSADA · 11:48 P.M.',title:'THEN THE SKY TURNED RED',text:'A Soviet rift tears across the garden. Five steel claws descend before the guards can raise their weapons.'},
    {image:'assets/cs-dogs-reactor-v2.jpg',kicker:'FIVE MINUTES LATER',title:'THE RANSOM',text:'Every screen in the palace shows all five dogs alive behind reactor glass. A Soviet countdown begins at five hours.'},
    {image:'assets/cs-milei-cockpit.jpg',kicker:'THE BÖHM-BAWERK · AIRBORNE',title:'ONE ROAD INTO THE PAST',text:'Hoppe finds the signal in Buenos Aires, 1951. Milei locks in the coordinates. The quiet hour is over.'}
  ],
  rand:[{image:'assets/cs-rand.jpg',kicker:'ENCRYPTED TRANSMISSION',title:'SOMEONE ABOARD IS LYING',text:'“Your flight plan was filed before the dogs were taken. The enemy did not find your route. He bought it.”'}],
  liveDogs:[{image:'assets/cs-dogs-reactor-v2.jpg',kicker:'HIJACKED CAMPUS SIGNAL',title:'FIVE HEARTBEATS',text:'The collar feed comes alive. All five dogs are breathing above the Kremlin reactor—and the countdown has lost an hour.'}],
  betrayal:[
    {image:'assets/cs-rand.jpg',kicker:'THE KEY MATCHES',title:'THE TRAITOR IS ON THE SHIP',text:'Every stolen coordinate carries Hoppe’s authorization mark. He did not lose the route. He sold it.'},
    {image:'assets/cs-stalin-statue.jpg',kicker:'MOSCOW · SECURE CHANNEL',title:'HOPPE CHOSE HIS FUTURE',text:'Stalin promised him a throne in the rewritten world. Payment required five hostages—and one open road to Milei.'}
  ],
  kennel:[{image:'assets/cs-dogs-reactor-v2.jpg',kicker:'ABOVE THE CORE',title:'FIVE PAWS HIT THE GLASS',text:'They know his footsteps. The reactor locks down, the timer turns red, and something enormous wakes below them.'}],
  marx:[{image:'assets/cs-marx-dissolves.jpg',kicker:'THE REACTOR OPENS',title:'THE MACHINE NEEDS A GHOST',text:'Marx tears free of the conduit and floods Stalin’s last shell. Steel stands up. History gets one final bad idea.'}],
  smith:[{image:'assets/cs-smith.jpg',kicker:'THE FREE PORT',title:'WELCOME TO THE DOLLARIZATION SHOP',text:'Adam Smith has one rule: spend what you saved, and save what matters.'}],
  homecoming:[{image:'assets/cs-homecoming-v2.jpg',kicker:'CASA ROSADA · SUNRISE',title:'THEY KNOW HIS FOOTSTEPS',text:'Five identical mastiffs hit the balcony at full speed. Milei drops the chainsaw just in time to catch all of them.'}],
  postCredits:[
    {image:'assets/cs-musk-news.jpg',kicker:'BREAKING NEWS',title:'ANOTHER HOSTAGE',text:'A new temporal crisis interrupts the victory broadcast.'},
    {image:'assets/cs-trump-call.jpg',kicker:'THE PHONE RINGS',title:'ONE MORE JOB',text:'“Javier, tremendous chainsaw. We have a situation.”'},
    {image:'assets/cs-new-adventure.jpg',kicker:'FREEDOM ARCADE',title:'TO BE CONTINUED',text:'The Böhm-Bawerk turns toward a new rift.'}
  ]
};
const seenScenes=new Set();
const seenTips=new Set();
let cinematicActive=false;
function playCutscene(key,onDone){const panels=CUTSCENES[key];if(!panels||seenScenes.has(key)){onDone?.();return}seenScenes.add(key);const wasPaused=state.paused;cinematicActive=true;state.paused=true;let index=0;const el=document.querySelector('#message');const render=()=>{const p=panels[index];el.innerHTML=`<div class="cinema"><img src="${p.image}" alt=""><div class="cinema-copy"><p class="eyebrow">${p.kicker}</p><h1>${p.title}</h1><p>${p.text}</p><div><button data-next>${index===panels.length-1?'CONTINUE':'NEXT'}</button><button data-skip>SKIP</button></div></div></div>`;el.classList.remove('hidden');el.querySelector('[data-next]').onclick=()=>{if(++index<panels.length)render();else finish()};el.querySelector('[data-skip]').onclick=finish};const finish=()=>{el.classList.add('hidden');cinematicActive=false;state.paused=wasPaused;onDone?.()};render()}
...
function update(dt){
  ...
  if(audio&&state.time>=state.musicNext){const notes=MUSIC[state.level],beat=state.musicNote++,boss=state.enemies.some(e=>TYPES[e.type].boss),note=notes[beat%notes.length];tone(note,.18,state.level===4?'sawtooth':'square',.011,'music');if(beat%4===0)tone(note/2,.11,'square',.018,'music');if(boss&&beat%2===1)tone(note*2,.06,'square',.009,'music');state.musicNext=state.time+(boss ? .19 : state.level===3 ? .24 : .28)}
...
function showShop(){if(!seenScenes.has('smith')){playCutscene('smith',showShop);return}let interest=0;if(!shopInterestApplied){shopInterestApplied=true;interest=Math.min(8,Math.floor(profile.dollars*.1*profile.interest));profile.dollars+=interest;if(interest)saveProfile()}const el=document.querySelector('#message'),next=state.level+1,hasNext=next<=4,cards=Object.entries(SHOP).map(([key,item])=>{const tier=profile[key],max=tier>=item.prices.length,cost=item.prices[tier];return `<button data-up="${key}" ${max?'disabled':''}>${item.name} · T${tier}/${item.prices.length}<small>${max?'MAXED':`${item.effect} · $${cost}`}</small></button>`}).join('');el.innerHTML=`<p class="eyebrow">ADAM SMITH'S</p><h1>DOLLARIZATION SHOP</h1><p>Stable money: <strong>$${profile.dollars}</strong> · Best score: <strong>${profile.highScore.toLocaleString()}</strong>${interest?` · Interest: <strong>+$${interest}</strong>`:''}</p><div class="shop">${cards}</div><p>SCORE ${state.score.toLocaleString()}</p><div>${shopPurchase?'<button data-refund class="secondary">REFUND LAST PURCHASE</button>':''}${hasNext?`<button data-next>CONTINUE TO LEVEL ${next}</button>`:''}<button data-replay class="secondary">REPLAY LEVEL ${state.level}</button></div>`;el.classList.remove('hidden');el.querySelectorAll('[data-up]:not(:disabled)').forEach(b=>b.onclick=()=>{const key=b.dataset.up,item=SHOP[key],cost=item.prices[profile[key]];if(profile.dollars>=cost){profile.dollars-=cost;profile[key]++;shopPurchase={key,cost};saveProfile();tone(660,.16,'square',.04);showShop()}else{tone(55,.12);b.animate([{transform:'translateX(-4px)'},{transform:'translateX(4px)'},{transform:'none'}],{duration:180})}});el.querySelector('[data-refund]')?.addEventListener('click',()=>{profile[shopPurchase.key]--;profile.dollars+=shopPurchase.cost;shopPurchase=null;saveProfile();tone(330,.12,'square',.03);showShop()});if(hasNext)el.querySelector('[data-next]').onclick=()=>{shopPurchase=null;el.classList.add('hidden');showIntro(next)};el.querySelector('[data-replay]').onclick=()=>{shopPurchase=null;el.classList.add('hidden');showIntro(state.level)}}
...
function drawFighter(o,isHero){
  ...
  const top=floorTop(),bottom=floorBottom();const bob=o.state==='walk'?Math.sin((o.walkPhase||0)*Math.PI/2)*1.5:0,arrival=!isHero?clamp(1-(o.entrance||0)/.42,0,1):1;let scale=t.scale*(.86+((o.y-top)/(bottom-top))*.14)*(.84+arrival*.16);
  ...
  ctx.save();ctx.translate(o.x,o.y+bob);if(!isHero&&o.state==='attack')ctx.rotate((o.face||1)*-.08);else if(o.state==='walk')ctx.rotate(clamp(o.vx||0,-180,180)/9000);const charging=!isHero&&o.state==='charge';ctx.scale((o.face||1)*scale*(charging?1.15:o.state==='attack'?1.06:1),scale*(charging?.88:o.state==='attack'?.94:1));
  if(im?.complete){const h=im.height,w=im.width;ctx.drawImage(im,-w/2,-h,w,h)}else{ctx.fillStyle=isHero?'#73c8e8':t.color;ctx.fillRect(-25,-80,50,80)}ctx.restore();ctx.globalAlpha=1;
  ...
  if(!isHero&&o.type==='tax'&&(o.stolen||0)>0){ctx.fillStyle='#ffd15a';ctx.strokeStyle='#07172d';ctx.lineWidth=3;ctx.font='bold 13px Arial';ctx.textAlign='center';ctx.strokeText(`$${Math.floor(o.stolen)}`,o.x,o.y-115);ctx.fillText(`$${Math.floor(o.stolen)}`,o.x,o.y-115);ctx.textAlign='left'}
  if(!isHero&&o.telegraph>0){const warning=o.attackKind==='charge'?'CHARGE!':o.type==='peron'?'DECREE!':o.type==='mecha_fdr'||o.type==='super_stalin'?'MISSILES!':'STRIKE!';ctx.strokeStyle='#ffd15a';ctx.lineWidth=5;ctx.beginPath();ctx.arc(o.x,o.y-50,48+Math.sin(state.time*22)*7,0,Math.PI*2);ctx.stroke();ctx.textAlign='center';ctx.fillStyle='#ffd15a';ctx.strokeStyle='#07172d';ctx.lineWidth=4;ctx.font='bold 16px Arial';ctx.strokeText(warning,o.x,o.y-125);ctx.fillText(warning,o.x,o.y-125);ctx.textAlign='left'}
}
...
function drawStageProgress(){const total=state.level===1?6:5,current=clamp(state.wave,1,total);ctx.fillStyle='#07172dcc';ctx.fillRect(18,78,224,47);ctx.fillStyle='#73c8e8';ctx.font='bold 9px Arial';ctx.fillText(`LEVEL ${state.level} · ENCOUNTER ${current}/${total}`,26,91);for(let i=0;i<total;i++){ctx.fillStyle=i<current-1?'#e0ad3b':i===current-1?'#c62f37':'#31445c';ctx.fillRect(26+i*(194/total),98,Math.max(12,178/total),7)}if(state.waveObjective){ctx.fillStyle='#f1e2bd';ctx.font='bold 8px Arial';ctx.fillText(state.waveObjective.label,26,118)}}
function drawHud(){
  ctx.fillStyle='#07172de8';ctx.fillRect(0,0,W,72);ctx.fillStyle='#f1e2bd';ctx.font='bold 18px Arial';ctx.fillText('MILEI',22,24);bar(20,34,230,22,hero.hp/hero.maxHp,'#c62f37');
  ...
  ctx.textAlign='right';ctx.fillStyle='#e0ad3b';ctx.font='bold 21px Arial';ctx.fillText(`PESOS $${Math.floor(state.pesos)}`,W-22,27);ctx.fillStyle='#f1e2bd';ctx.font='16px Arial';ctx.fillText(`SCORE ${state.score.toString().padStart(7,'0')}`,W-22,54);ctx.textAlign='left';
  ...
  if(state.multiplier>1){ctx.fillStyle='#ffd15a';ctx.font='bold 20px Impact';ctx.fillText(`×${state.multiplier.toFixed(1)}`,500,53)}
  ...
  drawStageProgress();
  const boss=state.enemies.find(e=>TYPES[e.type].boss);if(boss){const names={gremialista:'EL GREMIALISTA',evita:'EVITA',peron:'JUAN PERÓN',mecha_fdr:'MECHA-FDR',che_bike:'CHE GHOST',super_stalin:'SUPER STALIN'};ctx.fillStyle='#07172de8';ctx.fillRect(260,78,440,42);ctx.textAlign='center';ctx.fillStyle='#f1e2bd';ctx.font='bold 15px Arial';ctx.fillText(`${names[boss.type]} · PHASE ${boss.phase}`,480,94);bar(285,101,390,14,boss.hp/boss.maxHp,'#c62f37');ctx.textAlign='left'}
  if(state.bossMoveTime>0){ctx.globalAlpha=clamp(state.bossMoveTime*2,0,1);ctx.fillStyle='#07172dea';ctx.fillRect(325,124,310,30);ctx.strokeStyle='#ffd15a';ctx.lineWidth=2;ctx.strokeRect(325,124,310,30);ctx.textAlign='center';ctx.fillStyle='#ffd15a';ctx.font='bold 18px Impact';ctx.fillText(state.bossMove,480,146);ctx.textAlign='left';ctx.globalAlpha=1}
  if(state.bannerTime>0){ctx.globalAlpha=clamp(state.bannerTime,0,1);ctx.fillStyle='#07172ddd';ctx.fillRect(0,225,W,84);ctx.textAlign='center';ctx.fillStyle='#e0ad3b';ctx.font='bold 42px Impact';ctx.fillText(state.banner,W/2,279);ctx.textAlign='left';ctx.globalAlpha=1;}
  if(hero.hits>1&&hero.comboTime>0){ctx.textAlign='center';ctx.fillStyle='#ffd15a';ctx.strokeStyle='#07172d';ctx.lineWidth=5;ctx.font='bold 34px Impact';ctx.strokeText(`${hero.hits} HIT COMBO`,hero.x,hero.y-125);ctx.fillText(`${hero.hits} HIT COMBO`,hero.x,hero.y-125);ctx.fillStyle='#07172dcc';ctx.fillRect(hero.x-62,hero.y-113,124,7);ctx.fillStyle='#ffd15a';ctx.fillRect(hero.x-60,hero.y-111,120*clamp(hero.comboTime/1.15,0,1),3);ctx.textAlign='left'}
  if(state.waveGradeTime>0){ctx.globalAlpha=clamp(state.waveGradeTime,0,1);ctx.textAlign='center';ctx.fillStyle='#f1e2bd';ctx.strokeStyle='#07172d';ctx.lineWidth=6;ctx.font='bold 28px Impact';ctx.strokeText(state.waveGrade,W/2,335);ctx.fillText(state.waveGrade,W/2,335);ctx.textAlign='left';ctx.globalAlpha=1}
  ...
  if(state.paused){ctx.fillStyle='#07172ddd';ctx.fillRect(0,0,W,H);ctx.textAlign='center';ctx.fillStyle='#e0ad3b';ctx.font='bold 54px Impact';ctx.fillText('PAUSED',W/2,H/2);ctx.textAlign='left'}
}
